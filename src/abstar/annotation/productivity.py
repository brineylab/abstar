...
def assess_productivity(ab: Antibody) -> Antibody:
    ...
    if ab.v_call[:3] != ab.j_call[:3]:
        ab.productivity_issues.append(f"V/J locus mismatch ({ab.v_call} and {ab.j_call})")
    if ab.junction_aa:
        ...
        if ab.locus is not None:
            if ab.locus.upper() in ["IGH", "TRA", "TRD"]:
                if ab.junction_aa[-1] != "W":
                    ab.productivity_issues.append("junction does not end with conserved W")
            else:
                if ab.junction_aa[-1] != "F":
                    ab.productivity_issues.append("junction does not end with conserved F")
        else:
